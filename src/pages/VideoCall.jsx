...
const ORDER_TYPE = "vc";
const TERMS_KEY = "terms_videocall";
const MAX_TICKETS = 5;
...
export default function VideoCall() {
  ...
  async function fetchData() {
    try {
      ...
      const onlyWithVcFee = normalized.filter((m) => {
        const fg = getFeeByType(m, ORDER_TYPE);
        return !!fg?.id && (fg?.is_active ?? true);
      });

      const sortedMembers = onlyWithVcFee.sort((a, b) => {
      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        <div className="text-center mb-8 md:mb-12">
          ...
          <p className="text-base md:text-lg text-gray-400 max-w-2xl mx-auto">
            Siap Video Call bareng oshimu? Booking sekarang, proses cepat, aman, dan anti ribet!
          </p>
        </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        return (
                          <label key={opt.value} className={`flex flex-col gap-1 p-3 rounded-lg border-2 cursor-pointer transition-all select-none ${isSelected ? "border-primary-500 bg-primary-900/20" : "border-gray-700 bg-[#0A0E17] hover:border-gray-500"} ${!isOpen ? "cursor-not-allowed opacity-50" : ""}`}>
                            <div className="flex items-center gap-2">
                              <input type="radio" value={opt.value} {...register("account_type")} disabled={!isOpen} className="accent-primary-600 w-4 h-4" />
                              <span className={`font-semibold text-sm ${isSelected ? "text-primary-400" : "text-white"}`}>{opt.label}</span>
                            </div>
                    ...
                    {errors.account_type && <p className="text-sm text-red-400 mt-1 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.account_type.message}</p>}
                  </div>
