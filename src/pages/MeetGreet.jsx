...
function getCartItemByMember(cart, memberId) {
  return cart.find((item) => item.member_id === memberId);
}
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-800">
            {filteredMembers.map((m) => {
              ...
              const cartItem = getCartItemByMember(cart, m.id);
              const inCart = !!cartItem;
              const qty = cartItem?.quantity || 0;
              const maxReached = qty >= MAX_TICKETS;
              ...
              return (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <img
                        ...
                        loading="lazy"
                        onError={(e) => {
                          e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(m.name)}&background=3B82F6&color=fff`;
                        }}
                      />
                      ...
                      <div>
                        ...
                        {inCart && !full && (
                          <p className="text-xs text-amber-400 mt-1">
                            Sudah dipilih: {qty}/{MAX_TICKETS} tiket
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <td className="px-6 py-4 text-center">
                    <button
                      onClick={() => addToCart(m)}
                      disabled={!isOpen || full || maxReached}
                      title={
                        full
                          ? "Member ini sedang fullslot"
                          : maxReached
                            ? `Maksimal ${MAX_TICKETS} tiket untuk 1 member`
                            : ""
                      }
                      className={`inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg transition-all font-medium text-sm w-40 ${full
...
function MemberMobileCard({
  ...
  const cartItem = getCartItemByMember(cart, member.id);
  const inCart = !!cartItem;
  const qty = cartItem?.quantity || 0;
  const maxReached = qty >= MAX_TICKETS;
      ...
      <div className="flex items-center gap-3">
        <div className="relative shrink-0">
          <img
            ...
            loading="lazy"
            onError={(e) => {
              e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(member.name)}&background=3B82F6&color=fff`;
            }}
          />
...
export default function MeetGreet() {
  ...
  const addToCart = (member) => {
    ...
    const existingItem = cart.find((item) => item.member_id === member.id);

    if (existingItem) {
      const currentQty = Number(existingItem.quantity) || 1;

      if (currentQty >= MAX_TICKETS) {
        showToast(`Maksimal ${MAX_TICKETS} tiket untuk 1 member.`, "warning");
        return;
      }
      ...
      showToast(`Jumlah tiket ${member.name} menjadi ${currentQty + 1}.`, "success");
      return;
    }
    ...
    const activeEvents = TOUR_EVENTS.filter(
      (x) => targetWar === "all" || x.war === targetWar
    );
  ...
  const updateQuantity = (itemId, value) => {
    ...
    if (qty < 1) {
      showToast("Minimal 1 tiket untuk 1 member.", "warning");
      return;
    }
  ...
  async function onSubmit(data) {
    ...
    const invalidQtyItems = cart.filter((item) => {
      const qty = Number(item.quantity) || 1;
      return qty < 1 || qty > MAX_TICKETS;
    });

    if (invalidQtyItems.length > 0) {
      showToast(`Jumlah tiket per member minimal 1 dan maksimal ${MAX_TICKETS}.`, "error");
      return;
    }
    ...
    try {
      ...
      const noteLines = cart.map((item, idx) => {
        ...
        const qty = Number(item.quantity) || 1;
        const main = `${dateStr} | ${item.session} | ${item.member_name} | ${qty} tiket`;
      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        ...
        <div className="text-center mb-8 md:mb-12 anim-fade-up">
          ...
          <p className="text-base md:text-lg text-gray-400 max-w-2xl mx-auto">
            Dapatkan tiket Meet &amp; Greet bareng member favoritmu! Booking mudah, cepat, dan aman.
          </p>
        </div>
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-sm text-gray-400">
                              {formatCurrency(item.fee)} / member
                            </p>

                            <p className="text-xs text-gray-500 mt-1">
                              Maksimal {MAX_TICKETS} tiket.
                            </p>
                            ...
                            <div className="mt-3 flex items-center gap-2">
                              <button
                                ...
                                className="w-9 h-9 rounded-lg bg-[#12161F] border border-gray-700 text-white disabled:opacity-40 disabled:cursor-not-allowed hover:border-amber-500 transition-colors"
                              >
                                -
                              </button>
                              ...
                              <input
                                ...
                                className="w-16 h-9 text-center rounded-lg bg-[#12161F] border border-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-amber-500"
                              />

                              <button
                                type="button"
                                onClick={() => increaseQuantity(item.id)}
                                disabled={(Number(item.quantity) || 1) >= MAX_TICKETS}
                                className="w-9 h-9 rounded-lg bg-[#12161F] border border-gray-700 text-white disabled:opacity-40 disabled:cursor-not-allowed hover:border-amber-500 transition-colors"
                              >
                                +
                              </button>
                    ...
                    <div className="border-t border-gray-700 pt-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm text-gray-400">
                          {cart.length} Member • {totalTickets} Tiket
                        </span>
