...
export default function MeetGreet() {
  ...
  const { isOpen, loading: statusLoading } = useIsServiceOpen("meet_greet");
  ...
  async function fetchData() {
    try {
      ...
      const onlyWithMngFee = normalized.filter((m) => {
        const fg = getFeeByType(m, ORDER_TYPE);
        return !!fg?.id && (fg?.is_active ?? true);
      });

      const sortedMembers = onlyWithMngFee.sort((a, b) => {
  ...
  async function onSubmit(data) {
    ...
    const invalidBackups = cart.filter((item) => {
      ...
      if (item.backup_id === item.member_id && item.backup_date === item.date && item.backup_session === item.session) return true;
      return false;
    });

    if (invalidBackups.length > 0) {
      showToast("Cadangan dengan member dan tanggal yang sama harus memiliki sesi berbeda!", "error");
      return;
    }
    ...
    try {
      ...
      const noteLines = cart.map((item, idx) => {
        const main = `${item.date} | ${item.session} | ${item.member_name} | ${item.quantity} Tiket.`;
      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        ...
        <div className="text-center mb-8 md:mb-12">
          <h1 className="text-3xl md:text-5xl font-bold mb-3 md:mb-4 flex items-center justify-center gap-3">
            <Users className="w-8 h-8 md:w-10 md:h-10 text-primary-500" />
            Joki Meet &amp; Greet <span className="text-primary-500">JKT48</span>
          </h1>
          <p className="text-base md:text-lg text-gray-400 max-w-2xl mx-auto">
            Dapatkan kesempatan bertemu langsung dengan member favorit kamu! Booking mudah, proses cepat, dan terpercaya.
          </p>
        </div>

        <div className="max-w-7xl mx-auto mb-6">
          <ServiceStatusBanner serviceKey="meet_greet" />
        </div>
                  ...
                  <div className="max-h-[520px] overflow-y-auto">
                    <table className="w-full">
                      <thead className="sticky top-0 bg-[#1A1F2E] z-10">
                        <tr className="border-b border-gray-800">
                          ...
                          <th className="px-6 py-4 text-right text-sm font-semibold text-gray-300">Harga Meet &amp; Greet</th>
                    ...
                    {filteredMembers.length === 0 && (
                      <div className="text-center py-12 text-gray-500">
                        ...
                        <p className="text-sm mt-2">Pastikan member sudah di-assign ke fee group Meet &amp; Greet</p>
                      </div>
                    )}
                  </div>
                </div>
                      ...
                      <div key={item.id} className="bg-[#0A0E17] border border-gray-700 rounded-lg p-4 space-y-3">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-500 mt-1">1 member = 1 sesi</p>
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <label className="text-xs text-gray-300 mb-1 flex items-center gap-1"><Calendar className="w-3 h-3 text-gray-300" /> Tanggal</label>
                            <input type="date" value={item.date} onChange={(e) => updateCartItem(item.id, "date", e.target.value)} className="w-full px-3 py-2 text-sm bg-[#12161F] border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-primary-500" required />
                          </div>
                          <div>
                            <label className="text-xs text-gray-300 mb-1 flex items-center gap-1"><Clock className="w-3 h-3 text-gray-300" /> Sesi</label>
                            <select value={item.session} onChange={(e) => updateCartItem(item.id, "session", e.target.value)} className="w-full px-3 py-2 text-sm bg-[#12161F] border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-primary-500" required>
                        ...
                        <div className="border-t border-gray-700 pt-3 space-y-3">
                          <div className="flex items-center gap-2 text-xs text-gray-400"><Shield className="w-3 h-3" /><span>Member Cadangan (Opsional)</span></div>
                          <div>
                            <label className="text-xs text-gray-300 mb-1 block">Pilih Member</label>
                            <select value={item.backup_id} onChange={(e) => updateBackupMember(item.id, e.target.value)} className="w-full px-3 py-2 text-sm bg-[#12161F] border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-primary-500">
                              <option value="">Tidak ada cadangan</option>
                              {getAvailableBackupMembers().map((m) => (<option key={m.id} value={m.id}>{m.name}</option>))}
                            </select>
                          </div>
                          {item.backup_id && (
                            <>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                  <label className="text-xs text-gray-300 mb-1 flex items-center gap-1"><Calendar className="w-3 h-3 text-gray-300" /> Tanggal Cadangan</label>
                                  <input type="date" value={item.backup_date} onChange={(e) => updateCartItem(item.id, "backup_date", e.target.value)} className="w-full px-3 py-2 text-sm bg-[#12161F] border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-primary-500" required />
                                </div>
                                <div>
                                  <label className="text-xs text-gray-300 mb-1 flex items-center gap-1"><Clock className="w-3 h-3 text-gray-300" /> Sesi Cadangan</label>
                                  <select value={item.backup_session} onChange={(e) => updateCartItem(item.id, "backup_session", e.target.value)} className="w-full px-3 py-2 text-sm bg-[#12161F] border border-gray-700 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-primary-500" required>
                                    <option value="">Pilih</option>
                                    <option value="Sesi 1">Sesi 1</option>
                                    <option value="Sesi 2">Sesi 2</option>
                                    <option value="Sesi 3">Sesi 3</option>
                              ...
                              {item.backup_name && item.backup_date && item.backup_session && (
                                <div className="bg-[#12161F] border border-green-900/30 rounded p-2 text-xs">
                                  ...
                                  {item.backup_id === item.member_id && item.backup_date === item.date && item.backup_session === item.session && (
                                    <p className="text-red-400 text-xs mt-1">⚠️ Member dan tanggal sama, sesi harus berbeda!</p>
                                  )}
                                </div>
                              )}
                            </>
              ...
              <div className="p-4 md:p-6">
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  <div>
                    ...
                    <input type="text" {...register("customer_name")} disabled={!isOpen} className="w-full px-4 py-3 bg-[#0A0E17] border border-gray-700 rounded-lg text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:cursor-not-allowed" placeholder="Nama lengkap Anda" />
                    {errors.customer_name && <p className="text-sm text-red-400 mt-1 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.customer_name.message}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Username Twitter</label>
                    <input type="text" {...register("contact_twitter")} disabled={!isOpen} className="w-full px-4 py-3 bg-[#0A0E17] border border-gray-700 rounded-lg text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:cursor-not-allowed" placeholder="@username" />
                  </div>
                  ...
                  <div>
                    ...
                    <input type="email" {...register("contact_email")} disabled={!isOpen} className="w-full px-4 py-3 bg-[#0A0E17] border border-gray-700 rounded-lg text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:cursor-not-allowed" placeholder="email@example.com" />
                    {errors.contact_email && <p className="text-sm text-red-400 mt-1 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.contact_email.message}</p>}
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Password Akun JKT48 *</label>
                    <input type="password" {...register("password_jkt")} disabled={!isOpen} className="w-full px-4 py-3 bg-[#0A0E17] border border-gray-700 rounded-lg text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:cursor-not-allowed" placeholder="••••••••" />
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <input type="radio" value={opt.value} {...register("account_type")} disabled={!isOpen} className="accent-primary-600 w-4 h-4" />
                            <span className={`font-semibold text-sm ${watchAccountType === opt.value ? "text-primary-400" : "text-white"}`}>{opt.label}</span>
                          </div>
                  ...
                  <div className="flex items-start gap-3 pt-2">
                    <input type="checkbox" id="agree_terms" {...register("agree_terms")} disabled={!isOpen} className="mt-1 w-4 h-4 accent-primary-600 bg-[#0A0E17] border-gray-700 rounded disabled:cursor-not-allowed" />
                    <label htmlFor="agree_terms" className="text-sm text-gray-300">
                      Saya setuju dengan{" "}
                      <button type="button" onClick={() => setShowTerms(true)} className="text-primary-400 hover:text-primary-300 underline">syarat dan ketentuan</button>{" "}
                      yang berlaku
                    </label>
                  </div>
                  {errors.agree_terms && <p className="text-sm text-red-400 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.agree_terms.message}</p>}
